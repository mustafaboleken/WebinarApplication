...
const QuestionAnswerInputContainer = styled(Grid)(({ theme }) => ({
...
const MessageTextField = styled(TextField)(({ theme }) => ({
  "& .MuiOutlinedInput-root": {
    borderRadius: 30,
    backgroundColor: theme.palette.green[85],
  },
...
const QuestionAnswerInput = React.memo(() => {
  ...
  const sendMessage = () => {
    if (text) {
      antmedia.handleSendQuestion(text);
      settings?.handleSetQA({
        name: "You",
        message: text,
  ...
  return (
    <QuestionAnswerInputContainer container>
      <form
        onSubmit={(event) => {
