...
function AntMedia() {
  ...
  const [messageDrawerOpen, setMessageDrawerOpen] = useState(true);
  ...
  function handleSetQA(newQuestion) {
    setQuestions((oldQuestions) => {
      let lastQuestion = oldQuestions[oldQuestions.length - 1]; //this must remain mutable
      const isSameUser = lastQuestion?.name === newQuestion?.name;
      ...
      if (isSameUser && sentInSameTime) {
        //group the messages *sent back to back in the same timeframe by the same user* by joinig the new message text with new line
        lastQuestion.message = lastQuestion.message + "\n" + newQuestion.message;
        return [...oldQuestions]; // don't make this "return oldQuestions;" this is to trigger the useEffect for scroll bottom and get over showing the last prev state do
      } else {
  ...
  function handleSendQuestion(message) {
    if (myLocalData.streamId) {
  ...
  function handleNotificationEvent(obj) {
    ...
    if (notificationEvent != null && typeof notificationEvent == "object") {
      ...
      if (eventType === "CAM_TURNED_OFF") {
      ...
      } else if (eventType === "QUESTION_RECEIVED") {
        const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
        ...
        setQuestions((oldQuestions) => {
          let lastQuestion = oldQuestions[oldQuestions.length - 1]; //this must remain mutable
          const isSameUser = lastQuestion?.name === notificationEvent?.name;
          const sentInSameTime = lastQuestion?.date === notificationEvent?.date;

          if (isSameUser && sentInSameTime) {
            //group the messages *sent back to back in the same timeframe by the same user* by joinig the new message text with new line
            lastQuestion.message =
                lastQuestion.message + "\n" + notificationEvent.message;
            return [...oldQuestions]; // dont make this "return oldQuestions;" this is to trigger the useEffect for scroll bottom and get over showing the last prev state do
          } else {
            return [...oldQuestions, notificationEvent];
          }
        });
  ...
  antmedia.handleSendQuestion = handleSendQuestion;
            ...
            {leftTheRoom ? (
              ...
              >
                <>
                  ...
                  />
                  <MessageDrawer messageDrawerOpen={messageDrawerOpen} messages={messages} questions={questions} />
